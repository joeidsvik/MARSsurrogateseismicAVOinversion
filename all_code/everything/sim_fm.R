...
prior_gen_mean_tt <- function(tt, n_real=1, prior_mean, h = 1){
  # Input
  # tt - traveltime matrix
  # n_real - number of realisations to be generated
  # prior_mean - mean for the prior
  # h - tuning parameter
  #### Prior inputs ####
  phi_cl <- 15
  phi_sat <- 15
  ...
  #browser()
  mu_cl <-  matrix(prior_mean[,1], nrow = dim_tt[1], ncol = dim_tt[2])
  mu_o <-  matrix( prior_mean[,3], nrow = dim_tt[1], ncol = dim_tt[2])
  mu_g <- matrix( prior_mean[,2], nrow = dim_tt[1], ncol = dim_tt[2])
  
  x_rf_clay <- matrix(NA, nrow = my.dim, ncol = n_real)
  x_rf_sat_g <- matrix(NA, nrow = my.dim, ncol = n_real)
  x_rf_sat_o <- matrix(NA, nrow = my.dim, ncol = n_real)
...
sim_forward_model <- function(x_rf_sat_g, 
                              x_rf_sat_o, 
                              x_rf_clay, 
                              traveltimes){
  ...
  convrt <- 1.055
  # cementation depth
  depth_cem <- 2110 # 2000*convrt
  ref_depth <- 949.5 # 900*convrt
  ...
  #ref_depth <- 900*convrt  # testing with 0 ref depth
  # ref_depth <- ref_depth   # min and max porosity for the logistic transformation
  #depth included here
  n_param_mean <- 4
...
plot_prior_grid <- function(dim_row, dim_col, target, x, y, title = "", xlab, ylab, col = viridis(256)){
  zlim = c(min(target,na.rm = T),max(target, na.rm=T))
  grid_transform <- matrix(target, nrow = dim_row, ncol=dim_col, byrow = F)
  
  image.plot(x = x, y = -y, z = grid_transform, xlab =  xlab, ylab = ylab, main = title, zlim = zlim, asp = 1, col=col, axes = F)
  axis(1)
  box()
...
log_transf <- function(target_g, target_o, target_cl){
  sum_tot <- 1+ exp(target_g) + exp(target_o)
  sat_g <- exp(target_g)/sum_tot
  sat_o <- exp(target_o)/sum_tot
  sat_b <- 1/sum_tot
  v_cl <- exp(target_cl)/(1+exp(target_cl))
  
  return(list(sat_g=sat_g, sat_o=sat_o, sat_b= sat_b, v_cl = v_cl))
}
...
sim_forward_model_eval <- function(x_rf_sat_g, 
                              x_rf_sat_o, 
                              x_rf_clay, 
                              traveltimes, r0_obs, g_obs, Q_omega){
  # Specifying total dimension from traveltimes input
  if(is.null(dim(traveltimes))==TRUE){
    if(length(traveltimes)>=1){
      tot_dim <- length(traveltimes)
  ...
  temp_forward <- rp_likelihood_eval(r0_obs, g_obs, Q_omega, depth_cem, tot_dim, x_rf_poros_ss_log, x_rf_poros_sh_log, x_rf_sat_g_log, x_rf_sat_o_log, x_rf_sat_b_log, x_rf_clay_log,traveltimes,x_rf_poros_log) 
  #elast_param <- temp_forward[[1]]
  y_obs_sim_noise_free <- temp_forward[[2]]
  y_tilde <- temp_forward[[3]]
  
  return(temp_forward)
  #return(y_obs_sim_noise_free)
}

sim_forward_model_eval<- compiler::cmpfun(sim_forward_model_eval)
